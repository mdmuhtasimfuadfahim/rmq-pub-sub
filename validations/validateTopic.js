...
/**
 * Validates the provided topic using Joi schema.
 *
...
const validateTopic = (topic) => {
    ...
    const pubSubTopicSchema = Joi.object()
        .keys({
            topic: Joi.string().min(5).max(20).required().description('rabbimq produced topic'),
        })
        .unknown();
    ...
    if (error) {
        throw new Error("Invalid topic: The type of topic should be string or the lenght should not be less than 5 and more than 20");
    }
};
